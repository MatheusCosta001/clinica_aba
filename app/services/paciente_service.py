...
class PacienteService:
    ...
    def createOrUpdate(data):
        ...
        if not data.get("id"):
            paciente = PacienteRepo.add(p)
        else:
            db.session.commit()
            # reload to ensure we return a fresh instance with updated relationships/attributes
            paciente = PacienteRepo.getById(p.id)
        try:
            # ensure anonimizado flag is explicit and object is refreshed
            if getattr(paciente, 'anonimizado', None) is None:
                paciente.anonimizado = False
            db.session.refresh(paciente)
        except Exception:
            # if refresh fails for any reason, still return the paciente instance
            pass
        return paciente
