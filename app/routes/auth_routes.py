...
def admin_anonimizar_usuario(target_user_id):
    # ADM-only endpoint to anonymize another user
...
def admin_list_usuarios():
    # ADM-only page listing users with anonymize action
    if session.get('papel') != 'adm':
    ...
    # get existing especialidades to populate the filter select
...
def admin_edit_usuario(uid):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # update role if needed directly
